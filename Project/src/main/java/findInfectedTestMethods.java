...
//        changeInfo = args[2];//change_info.txt with changed methods
        String type = "-m";  //for test
        target = "E:\\study\\大三上\\自动化测试\\ClassicalAutomatedTesting_Final\\project_To_Analyse\\4-NextDay\\target";
        changeInfo="E:\\study\\大三上\\自动化测试\\ClassicalAutomatedTesting_Final\\project_To_Analyse\\4-NextDay\\data\\change_info.txt";
        try {
            t.initialize();
        } catch (Exception e) {
            e.printStackTrace();
...
//        }
        ...
        if(type.equals("-m")){//-m for method
            ...
            for(String s: t.changeMethods){
               t.findCGNode(t.cg.getFakeRootNode(),s);
                getAffectedTests(t.toFind, t);
            }
            for (String s:
                 t.outputRes) {
                fileWriter.write(s);
    ...
    int findCGNode(CGNode current_node,String tar){//dfs to get node which name is tar
...
//
    ...
    static void generateFiles(Set<String>dotFile,String name,String type) throws IOException {//from .dot set to .dot file
     ...
     static void getAffectedTests(CGNode cgNode, findInfectedTestMethods t){
        if (t.cgnode_set.contains(cgNode)){
            return ;
        }
        if(cgNode.getMethod() instanceof ShrikeBTMethod) {
            ...
            if (method.getDeclaringClass().getClassLoader().toString().equals("Application")) {
                for (CGNode node :
                    ...
                    if (temp[1].contains("Test")&&!temp[2].contains("<"))//select the tests
        ...
        t.cgnode_set.add(cgNode);
        for (CGNode node:
                Iterator2Iterable.make(t.cg.getPredNodes(cgNode))) {
            getAffectedTests(node,t);
        }
         t.cgnode_set.remove(cgNode);
